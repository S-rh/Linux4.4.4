 ...
 */
 // 创建新的内存映射是，除非用户指定了具体的地址，否则内核根据arch_get_unmapped_area找到一个适当的位置
 // 不同体系结构自定义arch_get_unmapped_area函数必须设置预处理器符号HAVE_ARCH_UNMAPPED_AREA
		...
		unsigned long len, unsigned long pgoff, unsigned long flags)
{
	...
	if (flags & MAP_FIXED)  // MAP_FIXED:表示映射将在固定地址创建
		return addr;
 ...
 */
// 寻找内存映射低端内存位置，通常从较低的内存位置开始，逐渐向较高的内存地址搜索
// 不同体系结构自定义arch_get_unmapped_area_topdown函数必须设置预处理器符号HAVE_ARCH_UNMAPPED_AREA_TOPDOWN
